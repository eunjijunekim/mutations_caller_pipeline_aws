class GatkCaller
  ...
  # Genotyper
  def self.call(log_dir, gatk, index_fa, read_bam, read_vcf, job_prefix, account,dbsnp_file)
  ...
  def self.coverage(log_dir, gatk, index_fa, read_bam, outfile_prefix, job_prefix, account)
  ...
  def self.recalibrate_bam(log_dir ,gatk, index_fa, read_bam, recal_file, job_prefix, account, dbsnp_file)
  ...
  # parallel not possible yet (1.6-13-g91f02df)
  def self.table_calibration(log_dir, gatk, index_fa, read_bam, recal_bam, recal_file, job_prefix, account)
  ...
  def self.prepare_realigne(log_dir, gatk, read_bam, index_fa, target_intervals, job_prefix, account, dbsnp_file, star)
  ...
  # Realignment
  # parallel not possible yet (1.6-13-g91f02df)
  def self.realigne(log_dir, gatk, read_bam, index_fa, target_intervals, realigned_bam, job_prefix, account, star)
